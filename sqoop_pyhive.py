...
class sqoop_pyhive:
    ...
    def create_flat_table(self, table_name):
        ...
        #table_statement=table_statement.replace(" NOT NULL","")
	...
	table_statement=table_statement+" ROW FORMAT DELIMITED STORED AS ORC tblproperties ('orc.compress'='"+self.config['SQOOP_COMPRESSION']+"')"
    ...
    def load_data_orc(self,table_name,primary_id,primary_value):
	 with pyhs2.connect(host=self.env['HIVE_HOST'],port=10000,authMechanism="PLAIN", user='sqoop2',password='',database=self.config['HIVE_DB']) as conn:
          with conn.cursor() as cur:
    ...
    def drop_files(self,table_name):
         with pyhs2.connect(host=self.env['HIVE_HOST'],port=10000,authMechanism="PLAIN", user='sqoop2',password='',database=self.config['HIVE_DB']) as conn:
          with conn.cursor() as cur:
            hive_sql="dfs -rm /user/sqoop2/"+table_name+"/*"
	    self.logger.info("RUNNING at HIVE: %s",hive_sql)
            cur.execute(hive_sql)
